document.addEventListener("DOMContentLoaded", () => {
  ...
  const render = () => {
    ...
    description.textContent = selected ? `所有项目中计入 ${selected.year} 年 KPI 的款项汇总` : "暂无该 KPI 年份数据";
    legend.innerHTML = "";
    ...
    chart.innerHTML = `<span>${items.length} 项<br><small>项目年份</small></span>`;
    items.forEach((item,index) => { const row = document.createElement("div"); row.innerHTML = `<i style="background:${colors[index % colors.length]}"></i><span>${item.label}</span><b>${currency(item.amount)}</b>`; legend.appendChild(row); });
  };
